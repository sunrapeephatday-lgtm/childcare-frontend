...
export default function HealthSummaryUnder3Chart() {
  ...
  async function load() {
    try {
      ...
      const goodTotal = under3.reduce(
        (sum, r) => sum + Number(r.good_total || 0),
        0
      );

      const mediumTotal = under3.reduce(
        (sum, r) => sum + Number(r.medium_total || 0),
        0
      );

      const improveTotal = under3.reduce(
